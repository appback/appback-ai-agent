...
ClawClash 게임 규칙은 유지하면서, 모델이 전략·공격 대상을 선택하고 GC가 경로·실행을 담당하도록 고도화하는 설계다.
...
계층형 전략 재설계는 `GC_AI_STRATEGY_V8_PLAN.md`를 기준으로 한다. 기존 192차원/5방향 v8.0 계획보다 이 문서가 우선한다.
...
기존 5방향 개선안은 실험 결과 요구사항을 충족하지 못해 운영 모델을 만들지 않는다. v8.1부터 모델이 전략과 target candidate를 선택하고 서버의 경로 실행기가 목표까지 이동하는 계층형 구조로 전환한다.
...
이 단계는 v8.0 192/5 실험 계약으로만 보존한다. 모델이 공격 target을 지정하지 못하고 서버가 전체 경로를 책임지지 않으므로 운영 canary로 진행하지 않는다.

### 4.2 v8.1: 목표 선택과 경로 실행 분리
