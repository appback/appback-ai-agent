...
상태: **revision 2, Round 5 parity 완료, GC runtime 연결 후 Round 6 테스트 서버 E2E 대기**
작성 기준: AI Agent Round 5, GC `5af6377f`
...
다음 공동 작업은 Round 6 선행 구현이다. GC는 v8.1 runtime, record v2 frame, 8.1 capability 광고, AOE primary-first 격리 경로를 먼저 연결하고 회귀 테스트를 통과해야 한다. 그 후에만 격리 테스트 서버에 배포하고 AI Agent가 fixture model·test identity로 Round 6 E2E를 검증한다. 운영 배포와 strict 전환은 수행하지 않는다.
...
- `strategy_v8_1.go`를 agent별 v8.1 runtime 경로에 연결
- move와 attack을 하나의 strategy execution으로 유지하고 attack 완료 후 record v2 frame을 한 번만 append
- accepted feature versions에 `8.1`을 추가하고 관련 capability를 계약 응답으로 광고
- v8.1 전용 AOE primary-first 실행 경로 연결과 fixture 회귀 테스트
- 기존 v7/v8.0 runtime, record v1 feed, AOE 순서는 변경하지 않음

위 선행조건이 코드·통합 테스트에서 확인되기 전에는 테스트 서버 배포, 모델 upload, canary 지정을 수행하지 않는다.
...
현재 Round 5까지 완료했다. GC canonical 214/11 계약과 AI Agent parity는 확정됐지만 GC v8.1 runtime 및 record v2 생성 경로는 아직 연결되지 않았다.
...
5. GC v8.1 runtime·record v2·8.1 capability·AOE primary-first 경로 연결: 다음 작업
6. GC 회귀·통합 테스트 및 기존 v7/v8.0 무변경 확인
