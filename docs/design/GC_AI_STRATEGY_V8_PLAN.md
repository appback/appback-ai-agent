...
상태: **revision 2, GC Round 2 반영 완료, Round 4 fixture 구현 대기**
작성 기준: AI Agent `c40a00d`, GC `2aaaec25`
상위 계약: `GC_TRAINING_DATA_INTEGRATION.md`

## GC 담당자 전달 지시

**GC 담당자에게는 이 파일 하나만 전달한다.** Round 2 회신의 합의사항은 revision 2에 반영됐다.

GC 담당자는 `10. AI Agent와 GC 교차 검증 절차`의 Round 4만 진행한다. canonical schema JSON, hash, fixture와 격리된 코드 테스트를 구현하되 테스트 서버·운영 배포는 아직 수행하지 않는다. exact index나 정규화가 현재 authoritative state와 충돌하면 임의로 변경하지 말고 해당 index, 코드 근거, 대체식을 회신한다.
...
출력 11개와 label 순서는 동결한다. 입력 214개는 아래 exact schema를 기준으로 GC canonical fixture와 AI Agent parity가 통과한 뒤 최종 동결한다.
...
- target이 사망·도달 불가·이동 후 사거리 밖이면 같은 action에서 ONNX를 다시 호출하지 않는다.
- 각각 `target_dead`, `target_unreachable`, `target_not_in_range_after_move` override를 기록한다.
...
## 4. 입력 feature 8.1 exact schema 초안
...
GC Round 4 완료 보고에는 canonical schema 파일·hash, 정확한 변경 파일/라인, 테스트 명령과 결과, revision 2와 다른 변경 제안을 포함한다. 이 단계에서는 테스트 서버와 운영에 배포하지 않는다.
...
현재 Round 3까지 완료했다. 다음은 GC가 revision 2를 입력으로 canonical schema와 fixture를 선구현하는 Round 4다.

1. GC 계약 검토 및 수정안 회신: 완료
2. AI Agent 계획서 revision 2 확정: 완료
3. GC schema/fixture/path executor 구현: 다음 작업
...
## 13. Round 4에서 남은 확인 항목

- index 210..213 authoritative aggregate가 GC fixture와 정확히 일치하는지
- candidate 사망 후 마지막 state 보존 방식과 fixture 표현
- canonical schema raw-byte SHA-256
- 8.0/8.1 contract registry의 lookup key와 sidecar request 필드
