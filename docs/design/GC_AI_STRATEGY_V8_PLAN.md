...
GC `7886dadd`에서 관리자 canary/activate/rollback 검사를 `featurecontract` registry 기반으로 변경했다. v8.0·v8.1 정상 계약은 허용하고 version/hash 교차 조합은 거부하는 회귀 테스트가 추가됐다. AI Agent `7c9ab97`은 feature 8.1에서 `capabilities.strategy_v8_1=true`가 아니거나 계약 조회에 실패하면 observe 모드에서도 fail-closed한다.
...
- API `sha256:0eca3d5b...`, inference `sha256:bd2f5d80...`, restart `0`
- 실제 214/11 fixture upload 및 canary pointer 지정 성공
- 격리 8인 게임에서 record v2 frame 21건 생성
- 21건 모두 feature 214, strategy mask 11, inference `ok`
- AI Agent consumer가 cursor API에서 session 1, frame 21, result 1을 수집하고 export 대상으로 검증
- scheduler를 다시 비활성화하고 임시 game/agent/model/admin user 및 model file을 모두 정리
...
각 후보는 독립된 ONNX, GC upload 전용 `meta.json`, `evaluation.json`을 가진다. 공통 계약은
`214 -> 128 -> 64 -> 11`, schema `gc-strategy-v8-214-r1`, 고정 seed `8107`, profile별
synthetic session 256개와 sample 2,048개다. `collector`는 powerup capability가 false이므로 제외한다.

오프라인 결과:

- teacher accuracy: balanced `85.3659%`, hunter `85.1220%`, survivor `86.3415%`, navigator `89.2683%`
- 최종 masked strategy 위반 및 inference failure: 모두 `0`
- 공통 2,048 관측에서 hunter attack `100%`, survivor flee `40.625%`, navigator explore `70.2148%`
