...
- **가중치**: 2단계 정책
  ...
  2. **Stay 부스트**: 현재 위치에서 공격 가능(f161=1)한 상황일 때:
     - action이 stay → 가중치 ×2.0 (공격 사거리 유지 보상)
     - action이 이동 → 가중치 ×0.5 (불필요한 이탈 억제)
- **공격은 자동** — 모델은 이동만 결정, 공격은 서버가 자동 처리
- 사거리 안에서 머무르면 서버가 자동 공격을 실행하므로, stay 부스트가 전투 효율을 높임
- f161은 무기별 사거리를 반영 (sword=인접, bow=거리2~4, spear=직선) → 무기 무관하게 동작
